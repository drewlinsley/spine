...
class DS(jit.ScriptModule):
    ...
    def forward(self, input, state):
        ...
        Ex = F.relu(ingate)
        Mn = F.relu(forgetgate)
        outgate = torch.sigmoid(cellgate)
        Ia_ex, Ia_fl = outgate.chunk(2, 1)
        Ia_both = torch.stack((Ia_ex, Ia_fl), -1)
        Ia_both = F.softmax(Ia_both, dim=-1)
        Ia = torch.cat((Ia_both[..., 0], Ia_both[..., 1]), 2)

        Mn = self.layernorm_c(Mn + Ex)

        Mn_pre = F.relu(Mn * Ia)
        Mn_post = outgate * Mn_post + (1 - outgate) * hx
        return Mn_post, (Mn_post, Mn_pre)
