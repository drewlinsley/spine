...
class DS(nn.Module):
    """DS with a linear readout shared across timesteps."""
    def __init__(
            self,
            input_size,
            hidden_size,
    ...
    def forward(self, input):
        ...
        if self.attention:
            ...
            input = torch.bmm(
                ...
                input.unsqueeze(0))
        # hidden_enc_h = self.init_hidden(input, self.gru_enc.hidden_size)
        # hidden_dec_h = self.init_hidden(input, self.gru_dec.hidden_size)
        hidden_enc = self.init_hidden(input, self.gru_enc.hidden_size)[:, 0]
        hidden_dec = self.init_hidden(input, self.gru_dec.hidden_size)[:, 0]
        outputs = torch.zeros((input.size(0), input.size(1), self.gru_dec.hidden_size)).to(input.device)
        tdiv = self.dist(loc=0., scale=1.)
        kl_div = 0
        for t in range(input.size(1)):
            it_input = input[:, t]
            # hidden_enc = hidden_enc_h[:, t]
            # hidden_dec = hidden_dec_h[:, t]
            output, hidden_enc = self.gru_enc(it_input, hidden_enc)
            output = torch.sigmoid(self.emb(output))
            div = self.dist(
                output[..., 0],
                output[..., 1])
            kl_div = kl_div + torch.distributions.kl.kl_divergence(div, tdiv).mean()
            output = div.rsample()[..., None]
            output, hidden_dec = self.gru_dec(output, hidden_dec)
            # if t < (input.size(1) - 1):
            #     hidden_enc_h[:, t + 1] = hidden_enc
            #     hidden_dec_h[:, t + 1] = hidden_dec
            outputs[:, t] = output
        outputs = self.out(outputs)
        return outputs, kl_div * self.alpha

    def init_hidden(self, d1, d2):
        """Initialize hidden states."""
        c1 = torch.zeros(d1.size(0), d1.size(1), d2, device=d1.device)
        # h1 = torch.zeros_like(c1)
        return c1  # , h1
        if self.batch_first:
            dim_zero = self.num_layers * self.multiplier
            c1 = torch.zeros(
                dim_zero, 1, d2, device=d1.device)
...
# class DSCell(jit.ScriptModule):
class DSCell(nn.Module):
    ...
    def forward(self, input, state):
        ...
        Ex = F.relu(ingate)
        Mn_gate = torch.sigmoid(cellgate)
        Mn = F.relu(forgetgate * Mn_gate)
        outgate = torch.sigmoid(outgate)
        Ia_both = F.softmax(outgate.view(-1, outgate.size(1) // 2, 2).contiguous(), -1)
        Ia = Ia_both.view(Ia_both.size(0), -1).contiguous()
        Mn_merge = self.layernorm_c(Mn + Ex)

        Mn_pre = Mn_merge * Ia
        Mn_post = outgate * F.relu(Mn_pre) + (1 - outgate) * hx
        # Mn_post = outgate * Mn_pre + (1 - outgate) * hx
        return Mn_post, Mn_post  # (Mn_post, Mn_pre)
